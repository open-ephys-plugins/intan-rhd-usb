...
ImpedanceMeter::ImpedanceMeter (DeviceThread* board_) : ThreadWithProgressWindow (
...
void ImpedanceMeter::factorOutParallelCapacitance (double& impedanceMagnitude, double& impedancePhase, double frequency, double parasiticCapacitance)
{
...
void ImpedanceMeter::empiricalResistanceCorrection (double& impedanceMagnitude, double& impedancePhase, double boardSampleRate)
{
    // First, convert from polar coordinates to rectangular coordinates.
    ...
    for (capRange = 0; capRange < 3; ++capRange)
    {
        ...
        for (channel = 0; channel < 32; ++channel)
        {
            ...
            for (stream = 0; stream < numdataStreams; ++stream)
            {
                ...
                if (board->chipId[stream] != CHIP_ID_RHD2164_B)
                {
                    measureComplexAmplitude (measuredMagnitude, measuredPhase, capRange, stream, channel, numBlocks, board->settings.boardSampleRate, actualImpedanceFreq, numPeriods);
                }
            }
    ...
    for (stream = 0; stream < board->evalBoard->getNumEnabledDataStreams(); ++stream)
    {
        ...
        for (channel = 0; channel < board->numChannelsPerDataStream[stream]; ++channel)
        {
            if (1)
            {
                ...
                empiricalResistanceCorrection (impedanceMagnitude, impedancePhase, board->settings.boardSampleRate);
...
void ImpedanceMeter::restoreBoardSettings()
{
    ...
    board->evalBoard->selectAuxCommandBank (Rhd2000EvalBoard::PortA, Rhd2000EvalBoard::AuxCmd3, board->settings.fastSettleEnabled ? 2 : 1);
    board->evalBoard->selectAuxCommandBank (Rhd2000EvalBoard::PortB, Rhd2000EvalBoard::AuxCmd3, board->settings.fastSettleEnabled ? 2 : 1);
    board->evalBoard->selectAuxCommandBank (Rhd2000EvalBoard::PortC, Rhd2000EvalBoard::AuxCmd3, board->settings.fastSettleEnabled ? 2 : 1);
    board->evalBoard->selectAuxCommandBank (Rhd2000EvalBoard::PortD, Rhd2000EvalBoard::AuxCmd3, board->settings.fastSettleEnabled ? 2 : 1);
